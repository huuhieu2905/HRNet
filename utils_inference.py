...
from model.hrnet import get_face_alignment_net
from config import config, merge_configs
import cv2


def get_model_by_name(hrnet_model, model_type='landmarks', device='cuda'):
    ...
    if hrnet_model == True:
        #You need to change this path to default HRNet Model
        checkpoint_path = '/home/hieunh/workspace/HRNetLandmark/HRNet-landmarks/hrnetv2_models/HR18-300W.pth'
        ...
        #You need to change this path to your own model
        checkpoint_path = 'output/300W/config/checkpoint_21.pth'
        model = get_face_alignment_net(config)
        state_dict = torch.load(checkpoint_path)
...
def get_lmks_by_img(model, img, output_size=(256, 256), rot=0):
    ...
    # img_crop = cv2.resize(img, (256, 256))
