...
from model.metric import decode_preds, compute_nme
...
class Trainer(object):
    ...
    def train(self, config, train_loader, model, critertion, optimizer, epoch, writer_dict):
        ...
        for i, (inp, target, meta) in enumerate(train_loader):
            ...
            #nme
            score_map = output.data.cpu() #Dau ra la heatmap
    ...
    def validate(self, config, val_loader, model, criterion, epoch, writer_dict):
        batch_time = Trainer()
        data_time = Trainer()
    ...
    def inference(self, config, data_loader, model):
        batch_time = Trainer()
        data_time = Trainer()
        losses = Trainer()
